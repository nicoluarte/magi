...
#+BEGIN_SRC R :results output :session 
  ...
  pkg <- c("dplyr", "ggplot2", "tidyverse", "corrplot", "Hmisc", "psycho",
           "broom", "gvlma", "pROC", "caret", "MASS", "effects", "car",
           "ResourceSelection", "neuralnet", "lmtest", "pROC")
  invisible(lapply(pkg, library, character.only = TRUE))
                                           # load database
  setwd("/home/nicoluarte/Downloads")
  df <- data.frame(read.csv("data_fondecyt.csv"))
  ...
  df$Pupil <- as.numeric(df$Pupil)
  df.hot <- dummyVars(Valence ~ ., data = df)
  df.use <- data.frame(predict(df.hot, newdata = df))
  head(df.use)
